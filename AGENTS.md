...
- STATUS shows current/maximum shells, current/maximum electricity, Fusion Cutter damage and range, and scrap. MATERIALS shows only titanium and uranium. ITEMS lists merchant purchases and body modifiers; pressing `O` on a modifier cycles it through the current body's compatible mounts and then back to storage.
...
- Opening the deep Vault route removes both `under-cache` and the low `vault-high` ceiling, then installs `vault-depth-access`. Preserve the 200-unit shaft between that wall and `vault-depth-floor-east`; a Wall-Climb-equipped player must physically climb from `vault-depth-floor-west` and land on `vault-deep-drop-one`. The two later climb walls are 200 and 220 units tall, exceed the basic jump, and lead through west/east galleries before the final boss drop.
...
- Merchant services are data-driven: `damageUpgrade` adds `+1` slash damage through four forge tiers; `healthUpgrade` adds one maximum shell through three tiers; `energyUpgrade` adds 25 maximum electricity through three tiers; `modifierShop` sells its configured reusable modifier stock; `internalSlot` adds reduced-effect internal body bays through three tiers; and `relicShop` sells configured passive relic stock. Health and energy merchants may append relic rows to their normal catalogs. All purchases persist for the run and appear in ITEMS.
- `RELICS` contains exactly eight distinct passive items: Mender Loop, Impact Damper, Feedback Dynamo, Execution Coil, Arc Retort, Kinetic Memory, Salvage Lens, and Corpse Key. Relics never occupy body mounts, and every effect is always active after purchase.
...
- The route can open only when both `player.abilities.wallClimb` and `vaultBossArena.cleared` are true. Opening it replaces the intact Warden floor with two safe floor pieces, an entrance wall, and a 200-unit hatch. The route then alternates mandatory wall climbs and broad galleries before reaching the arena floor at Y 2550.
...
- The standard body owns four full-strength mounts: `shell`, `core`, `legs`, and `weapon` for the Fusion Cutter. A modifier can occupy only one mount at a time. In ITEMS, `O` cycles the selected modifier through only its compatible mounts and storage; moving one modifier onto an occupied mount displaces the previous modifier. Some modifiers deliberately have no weapon effect and must skip that mount.
