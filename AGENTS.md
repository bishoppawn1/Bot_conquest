...
- `RELICS` contains exactly eight distinct passive items: Mender Loop, Impact Damper, Feedback Dynamo, Execution Coil, Arc Retort, Kinetic Memory, Salvage Lens, and Corpse Key. Relics have no dedicated slots. A purchased relic is inactive in storage, may occupy any of the four normal `shell`, `core`, `legs`, or `weapon` mounts, shares those mounts with modifiers, and retains the same effect in every valid mount.
