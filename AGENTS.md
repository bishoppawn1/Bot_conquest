...
- Killing enemies awards scrap based on archetype. The Grand Exchange `EDGE FORGE` offers four run-persistent `+1` primary-slash damage upgrades costing 500, 900, 1500, and 2400 scrap. Other authorized scrap sinks are the three-tier shell reinforcement, capacitor, and internal-bay services plus the three configured body modifiers; do not invent additional sinks without a new design decision.
...
- The current world spans X `0–14500` and Y `-1000–2240`. `REGIONS` contains eight contiguous regions connected by seven non-blocking `REGION_GATES`. Core Bastion spans X `5890–8400` and includes the former Relay Concourse space as one map region.
...
- Opening the deep Vault route removes both `under-cache` and the low `vault-high` ceiling, then installs the short `vault-depth-access` lip. Preserve at least 80 units of headroom above that lip and the 90-unit drop shaft to its right; a Wall-Climb-equipped player must physically climb from `vault-depth-floor-west` and land on `vault-deep-drop-one`.
