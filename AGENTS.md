...
- The start screen offers an explicit God Mode checkbox for playtesting. It grants every ability, permanent merchant tier, shell, bay, modifier, relic, and map; enemy hits never remove a shell, spikes and falls reset safely without shell loss, and attacks and repairs never consume electricity. Ordinary unchecked runs must retain normal progression.
...
- `REAR_WORKS_BLOCKS` forms the late-game chamber above and behind the initial Rusted Verge spawn. The 880-unit shaft from `rear-works-climb-base` to the enclosed launch shelf must require Wall Climb. A 300-unit empty span then requires Dash to reach `rear-works-floor`; Wall Climb alone must fall safely onto the old Verge roof, and Dash alone must never reach the launch. Preserve the chamber's two platform heights, four ordinary encounters, supported spike bed, scrap cache, and reversible return.
...
- `AREA_EXTENSION_BLOCKS` adds three connected longline surfaces inside every named region without changing the fixed world bounds. Each region's extension carries an ordinary encounter, a scrap cache, and a supported platform hazard, and must not obstruct any previously tested route.
...
Use `http://127.0.0.1:4173/?debug=crown-boss` for the active Crown Dynamo, `http://127.0.0.1:4173/?debug=field-annex` for its cleared Field-gated annex, `http://127.0.0.1:4173/?debug=slicer-gate` for the intact Volt-gated reward wall, `http://127.0.0.1:4173/?debug=slicer` for the equipped alternate shell, `http://127.0.0.1:4173/?debug=slicer&panel=items` for its mount diagram, `http://127.0.0.1:4173/?debug=shell-menu` for the rest selector, `http://127.0.0.1:4173/?debug=dash-routes` for the distributed Dash backtracking pockets, and `http://127.0.0.1:4173/?debug=rear-works` for the Wall-Climb-and-Dash chamber behind the initial spawn.
