...
test('Dash reaches optional pockets in ordinary regions that the basic jump cannot',()=>{
  const reaches=(sourceId,targetId,dash)=>{for(const inset of[0,20,40]){const g=new Game();g.enemies=[];g.traps=[];g.junkPiles=[];if(dash)g.unlockAbility('dash');const source=g.platforms.find(block=>block.id===sourceId),target=g.platforms.find(block=>block.id===targetId),direction=Math.sign((target.x+target.w/2)-(source.x+source.w/2)),move=direction>0?'right':'left';Object.assign(g.player,{x:direction>0?source.x+source.w-g.player.w-inset:source.x+inset,y:source.y-g.player.h,onGround:true,jumps:1,facing:direction,vx:0,vy:0});g.setInput({[move]:true,jump:true,dash});g.update(1/60);g.setInput({[move]:true,jump:false,dash:false});for(let frame=0;frame<180;frame++){g.update(1/60);if(supportingPlatform(g.player,g.platforms,3)===target)return true;}}return false;};
  for(const [source,target] of [['west-bridge','dash-verge-landing'],['dash-gauntlet-launch','dash-gauntlet-landing'],['exchange-high-cache','dash-exchange-cache']]){assert.equal(reaches(source,target,false),false,`${target} should require Dash`);assert.equal(reaches(source,target,true),true,`${target} cannot be reached with Dash`);}
});
...
test('two slashes destroy a basic enemy in front of the player',()=>{const g=new Game();tick(g,60);const e=g.enemies[0];g.enemies=[e];e.x=g.player.x+g.player.w+8;e.y=g.player.y;g.player.attackHits=new Set();g.resolvePrimaryAttack();assert.equal(e.dead,false);g.player.attackHits=new Set();g.resolvePrimaryAttack();assert.equal(e.dead,true);});
test('slash does not damage an enemy behind the player',()=>{const g=new Game();tick(g,60);const e=g.enemies[0];g.enemies=[e];e.x=g.player.x-35;e.y=g.player.y;g.player.aimX=1;g.player.aimY=0;press(g,'attack');assert.equal(e.dead,false);assert.equal(e.health,6);});
