...
test('touching a wall does not suppress the starting jump before wall climb is unlocked',()=>{const g=new Game();g.player.x=380;g.player.y=610;g.player.vx=300;tick(g);assert.equal(g.player.onWall,1);press(g,'jump');assert.ok(g.player.vy<0);});
