...
Relics are permanent passive merchant items and the single name for this item category. They are separate from installable body modifiers and do not consume body mounts. Every purchased relic appears in ITEMS with its always-active effect.
...
Wall Climb leads into a Crownworks chamber built around a central shaft and two separated floor masses. Entering either side closes two horizontal gate pieces across the shaft. The thirty-health Crown Dynamo alternates between two high anchor positions and can be damaged only during its exposed phase, requiring the player to climb the chamber perches and attack upward instead of fighting entirely from the floor.
...
Pressing `O` near the merchant opens a large, simulation-pausing catalog rather than immediately purchasing an item. The catalog displays every offer in that shop with its full effect, scrap cost, and available, unaffordable, prerequisite-locked, or owned state. Each specialist displays a short specialty description and stocks synergistic passives: Shell Archive supports survivability, Capacitor Exchange supports charge recovery, and Edge Forge supports cutter output. Unsupported mounts are omitted from an offer instead of implying that every modifier can be used offensively. `W` and `S` move through the list, `O` purchases the selected available offer, and `I` closes the catalog. The current services sell health tiers, electricity-capacity tiers, three reusable body modifiers, three reduced-power internal bays, eight passive relics, and the Edge Forge's four slash upgrades. Response Forge and Salvage Curator remain additional relic discoveries. Merchants still display carried special materials, while titanium/uranium recipes remain pending.

Enemies, junk, and secrets occupy the upper networks, the deep vault, and the ability-gated regions—not only the lower left-to-right floor. The map contains ten spike gaps, three exhaustible conduits, twenty ordinary junk piles, two Volt-powered seals, one Field-only seal, one optional mini-boss encounter, four full boss encounters, and two end alcoves. The Sunken Vault uses a darker blue-black background, Shard Gauntlet uses red hazard silhouettes, and Grand Exchange uses warm industrial accents.
...
The deep Sunken Vault becomes physically accessible once the Abyss Warden is dead and Wall Climb is owned. Opening it retracts both the old boss-floor mass and the low `vault-high` cap. The replacement floor leaves a 200-unit shaft to the right of a 120-unit climb wall, wide enough for deliberate entry rather than accidental edge clipping. Below it, two broad side galleries, two mandatory climb walls, two ordinary encounters, and three tested controlled drops form an explorable route before the boss arena.
