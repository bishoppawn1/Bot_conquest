...
The standard body has four full-strength modifier mounts: shell, core, legs, and the Fusion Cutter. A purchased modifier is reusable and occupies at most one mount. On the ITEMS page, selecting a modifier and pressing `O` cycles it from storage through only its compatible mounts and back to storage. If the destination is occupied, its previous modifier returns to storage. Modifiers are deliberately not universal: Aegis Filament and Reactive Governor cannot be installed on the cutter.
...
Eight distinct relics are distributed across four merchants. Shell Archive sells Mender Loop (750 scrap, repairs cost 5 less electricity) and Impact Damper (900, half knockback after damage). Capacitor Exchange sells Feedback Dynamo (850, taking damage restores 12 electricity) and Execution Coil (1,050, enemy kills restore 8 electricity). The hidden Response Forge sells Arc Retort (1,100, taking damage deals 2 damage to enemies within 120 units) and Kinetic Memory (1,250, the next Fusion Cutter slash after damage deals +2). The hidden Salvage Curator sells Salvage Lens (800, ordinary enemies award +3 scrap) and Corpse Key (950, recovery wrecks have one health). These effects are conditional and combinable rather than ascending copies of one stat.
...
Pressing `O` near the merchant opens a large, simulation-pausing catalog rather than immediately purchasing an item. The catalog displays every offer in that shop with its full effect, scrap cost, and available, unaffordable, prerequisite-locked, or owned state. Unsupported mounts are omitted from an offer instead of implying that every modifier can be used offensively. `W` and `S` move through the list, `O` purchases the selected available offer, and `I` closes the catalog. The current services sell health tiers, electricity-capacity tiers, three reusable body modifiers, three reduced-power internal bays, eight passive relics, and the Edge Forge's four slash upgrades. Shell Archive and Capacitor Exchange mix two relics into their existing catalogs; the Response Forge and Salvage Curator are additional discoveries devoted to relics. Merchants still display carried special materials, while titanium/uranium recipes remain pending.
