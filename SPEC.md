...
The current map is a complete geometric reset and must not derive its layout from the discarded access-path prototype. It spans X 0–14,500 and Y -1,000–2,700. Seventeen foundation pieces form the lower traversal network. Within the Sunken Vault, five foundations descend from Y 680 to Y 820 and rise again in 70-unit increments. A reversible branch continues to Y 1,040 before one deliberate drop reaches the Abyss Warden floor at Y 1,120. The gate-controlled Warden escape uses three tested basic-jump platforms; no accidental early drop is allowed to strand the player. A Wall-Climb loft extends above the original Vault, while the post-Warden climb galleries and controlled descents extend to the Rift Stalker at Y 2,550.
...
Enemies, junk, and secrets occupy the upper networks, the deep vault, and the ability-gated regions—not only the lower left-to-right floor. The map contains ten spike gaps, three exhaustible conduits, sixteen ordinary junk piles, two powered seals, one optional mini-boss encounter, three full boss encounters, and two end alcoves. The Sunken Vault uses a darker blue-black background, Shard Gauntlet uses red hazard silhouettes, and Grand Exchange uses warm industrial accents.

Crownworks continues well above its original roof line. Wall Climb carries the player through a narrow structural opening into a broad, enclosed upper works with a heavy roof, two floor masses, climbable interior braces, combat, salvage perches, and a safe return through the same shaft. It is playable exploration space rather than decorative ceiling geometry.
