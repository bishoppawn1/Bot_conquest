...
export class Renderer {
  ...
  drawInventory(game) {
    ...
    if(page==='map')this.drawInventoryMap(game,ctx);else{
      const titles={status:'CORE STATUS',materials:'SPECIAL MATERIALS',items:'ITEMS + RELICS'},rows=page==='status'?[['CORE SHELLS',`${game.player.lives} / ${game.player.maxLives}`],['ELECTRICITY',`${Math.round(game.player.electricity)} / ${game.player.maxElectricity}`],['FUSION CUTTER',`${game.player.primaryDamage} DMG // ${game.player.primaryRange} RANGE`],['SCRAP',String(game.player.scrap)]]:page==='materials'?[['TITANIUM',String(game.player.materials.titanium)],['URANIUM',String(game.player.materials.uranium)]]:game.inventoryItemRows(),visibleCount=6,start=page==='items'?Math.max(0,Math.min(game.inventorySelection-visibleCount+1,rows.length-visibleCount)):0;
      ...
      if(page==='items'){ctx.fillStyle='#71838a';ctx.font='9px Space Mono';ctx.fillText(`STANDARD BODY // SHELL + CORE + LEGS + FUSION CUTTER // ${game.player.body.internalSlots.length} INTERNAL BAYS`,150,610);if(rows.length>visibleCount){ctx.textAlign='right';ctx.fillText(`${game.inventorySelection+1} / ${rows.length}`,1130,610);ctx.textAlign='left';}}
    }
    ctx.restore();
  }
  ...
  drawMerchantMenu(game) {
    ...
    ctx.fillStyle='#65757a';ctx.font='9px Space Mono';ctx.fillText('FULL INVENTORY // SELECT AN OFFER TO PURCHASE',140,180);
