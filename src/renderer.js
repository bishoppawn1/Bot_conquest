...
export class Renderer {
  ...
  drawEquipmentInventory(game,ctx) {
    ...
    ctx.fillStyle='#ffffff';ctx.font='700 20px Space Mono';ctx.fillText('ACQUIRED EQUIPMENT',listX,200);ctx.fillStyle='#71838a';ctx.font='12px Space Mono';ctx.fillText('MODIFIERS + RELICS REQUIRE A SLOT',listX,214);
    ...
    const panel={x:535,y:190,w:600,h:310};ctx.fillStyle='rgba(10,18,22,.78)';ctx.fillRect(panel.x,panel.y,panel.w,panel.h);ctx.strokeStyle='#33454c';ctx.strokeRect(panel.x,panel.y,panel.w,panel.h);ctx.fillStyle='#ffffff';ctx.font='700 18px Space Mono';ctx.fillText(game.player.body.name,panel.x+20,panel.y+28);ctx.fillStyle='#71838a';ctx.font='12px Space Mono';ctx.fillText('CURRENT SHELL // MODIFIER, INTERNAL + RELIC BAYS',panel.x+20,panel.y+46);
    ...
    const chosen=game.equipmentItemIndex===null?null:game.player.purchasedItems[game.equipmentItemIndex],target=game.selectedEquipmentTarget(),active=Boolean(chosen),bodySlots=game.bodySlots(),internalSlots=game.player.body.internalSlots,lockedInternal=Array.from({length:3},(_,index)=>internalSlots[index]??{id:`locked-internal-${index+1}`,part:'locked',label:`INTERNAL BAY ${index+1}`,locked:true}),slotBoxes=[{slot:bodySlots.find(slot=>slot.id==='shell'),x:555,y:270,w:150},{slot:bodySlots.find(slot=>slot.id==='core'),x:555,y:330,w:150},{slot:bodySlots.find(slot=>slot.id==='legs'),x:555,y:390,w:150},{slot:bodySlots.find(slot=>slot.id==='weapon'),x:955,y:330,w:150},...lockedInternal.map((slot,index)=>({slot,x:720+index*130,y:250,w:120})),...game.relicSlots().map((slot,index)=>({slot,x:720+index*130,y:445,w:120}))];
    const drawSlot=({slot,x,y,w=150})=>{if(!slot)return;const occupant=items.find(item=>item.equippedSlot===slot.id),compatible=!active||(!slot.locked&&((chosen.kind==='relic'&&slot.part==='relic')||(chosen.kind==='modifier'&&Boolean(game.modifierEffect(chosen,slot))))),selected=active&&target?.id===slot.id,color=slot.part==='relic'?'#d6ff3f':'#75f5ff';ctx.fillStyle=selected?'rgba(214,255,63,.18)':compatible?`${color}14`:'rgba(255,255,255,.02)';ctx.fillRect(x,y,w,42);ctx.strokeStyle=selected?'#ffffff':compatible?color:'#354047';ctx.lineWidth=selected?3:1;ctx.strokeRect(x,y,w,42);ctx.fillStyle=compatible?'#ffffff':'#4c5b60';ctx.font='700 12px Space Mono';ctx.fillText(slot.label.replace('INTERNAL','INT'),x+7,y+15);ctx.fillStyle=occupant?'#d6ff3f':'#65757a';ctx.font='12px Space Mono';ctx.fillText(occupant?occupant.name.slice(0,15):slot.locked?'LOCKED':compatible?'EMPTY':'INCOMPATIBLE',x+7,y+32);};slotBoxes.forEach(drawSlot);
