...
export class Renderer {
  ...
  drawJunkPile(pile) {
    ...
    if(pile.material){ctx.shadowBlur=12;ctx.shadowColor=materialColor;ctx.fillStyle=materialColor;ctx.font='700 12px Space Mono';ctx.fillText(pile.material.type==='uranium'?'U':'TI',8,13);ctx.shadowBlur=0;}if(pile.minimumDamage>1||pile.requires){ctx.strokeStyle='#75f5ff';ctx.lineWidth=3;ctx.strokeRect(3,3,pile.w-6,pile.h-6);ctx.fillStyle='#75f5ff';ctx.font='700 13px Space Mono';ctx.fillText(pile.requires==='field'?'FIELD':'VOLT',pile.w/2-(pile.requires==='field'?16:13),pile.h/2+3);}if(damage>0){ctx.strokeStyle='#d6ff3f';ctx.globalAlpha=.35+damage*.5;ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(pile.w*.52,4);ctx.lineTo(pile.w*.45,pile.h*.42);ctx.lineTo(pile.w*.58,pile.h*.7);ctx.stroke();}ctx.restore();
  }
  ...
  drawEquipmentInventory(game,ctx) {
    ...
    ctx.fillStyle='#ffffff';ctx.font='700 20px Space Mono';ctx.fillText('ACQUIRED EQUIPMENT',listX,200);ctx.fillStyle='#71838a';ctx.font='12px Space Mono';ctx.fillText('MODIFIERS INSTALL // RELICS STAY PASSIVE',listX,214);
    rows.slice(start,start+visibleCount).forEach((item,index)=>{const actual=start+index,ry=listY+index*rowH,selected=actual===game.inventorySelection;ctx.fillStyle=selected?'rgba(117,245,255,.13)':'rgba(255,255,255,.025)';ctx.fillRect(listX,ry,listW,rowH-5);ctx.strokeStyle=selected?'#75f5ff':'#27363c';ctx.lineWidth=selected?2:1;ctx.strokeRect(listX,ry,listW,rowH-5);this.drawItemIcon(item,listX+9,ry+7,28);ctx.fillStyle=selected?'#ffffff':'#8b9ba0';ctx.font='700 12px Space Mono';ctx.fillText(item.name,listX+48,ry+18);ctx.fillStyle=item.kind==='modifier'?'#75f5ff':item.kind==='relic'?'#d6ff3f':'#ffb85c';ctx.font='12px Space Mono';ctx.fillText(item.kind==='modifier'?(item.equippedSlot?'INSTALLED':'MODIFIER // STORAGE'):item.kind==='relic'?'PASSIVE RELIC':'PERMANENT',listX+48,ry+34);});
    const panel={x:535,y:190,w:600,h:310};ctx.fillStyle='rgba(10,18,22,.78)';ctx.fillRect(panel.x,panel.y,panel.w,panel.h);ctx.strokeStyle='#33454c';ctx.strokeRect(panel.x,panel.y,panel.w,panel.h);ctx.fillStyle='#ffffff';ctx.font='700 18px Space Mono';ctx.fillText(game.player.body.name,panel.x+20,panel.y+28);ctx.fillStyle='#71838a';ctx.font='12px Space Mono';ctx.fillText('CURRENT SHELL // LIVE MODIFIER MOUNTS',panel.x+20,panel.y+46);
    const bodyX=820,bodyY=345;ctx.fillStyle='#1d2b31';this.polygon([[bodyX-82,bodyY-36],[bodyX-58,bodyY-58],[bodyX+55,bodyY-52],[bodyX+84,bodyY-24],[bodyX+70,bodyY+38],[bodyX-70,bodyY+42]],ctx.fillStyle);ctx.strokeStyle='#75f5ff';ctx.lineWidth=3;ctx.strokeRect(bodyX-48,bodyY-28,96,54);ctx.fillStyle='#071015';ctx.fillRect(bodyX-35,bodyY-13,70,25);for(const eyeX of[-20,0,20]){ctx.fillStyle='#ff493f';ctx.beginPath();ctx.arc(bodyX+eyeX,bodyY,5,0,Math.PI*2);ctx.fill();}this.line(bodyX-45,bodyY+34,bodyX-78,bodyY+72,'#87969b',5);this.line(bodyX,bodyY+34,bodyX-10,bodyY+78,'#87969b',5);this.line(bodyX+45,bodyY+34,bodyX+78,bodyY+72,'#87969b',5);this.line(bodyX+72,bodyY-8,bodyX+125,bodyY-8,'#dce6dd',6);
    const chosen=game.equipmentItemIndex===null?null:game.player.purchasedItems[game.equipmentItemIndex],target=game.selectedEquipmentTarget(),active=Boolean(chosen),internalPositions=[{x:955,y:235},{x:955,y:385},{x:955,y:435}],slotBoxes=[{slot:game.bodySlots().find(slot=>slot.id==='shell'),x:555,y:250},{slot:game.bodySlots().find(slot=>slot.id==='core'),x:555,y:325},{slot:game.bodySlots().find(slot=>slot.id==='legs'),x:555,y:400},{slot:game.bodySlots().find(slot=>slot.id==='weapon'),x:955,y:325},...game.player.body.internalSlots.map((slot,index)=>({slot,...internalPositions[index]}))];
    const drawSlot=({slot,x,y})=>{if(!slot)return;const occupant=items.find(item=>item.kind==='modifier'&&item.equippedSlot===slot.id),compatible=!active||Boolean(game.modifierEffect(chosen,slot)),selected=active&&target?.id===slot.id;ctx.fillStyle=selected?'rgba(214,255,63,.18)':compatible?'rgba(117,245,255,.08)':'rgba(255,255,255,.02)';ctx.fillRect(x,y,155,48);ctx.strokeStyle=selected?'#d6ff3f':compatible?'#75f5ff':'#354047';ctx.lineWidth=selected?3:1;ctx.strokeRect(x,y,155,48);ctx.fillStyle=compatible?'#ffffff':'#4c5b60';ctx.font='700 12px Space Mono';ctx.fillText(slot.label,x+10,y+17);ctx.fillStyle=occupant?'#d6ff3f':'#65757a';ctx.font='12px Space Mono';ctx.fillText(occupant?occupant.name.slice(0,18):compatible?'EMPTY':'INCOMPATIBLE',x+10,y+35);};slotBoxes.forEach(drawSlot);
    const storage={id:null,part:'storage',label:'STORAGE'},storageSelected=active&&target?.part==='storage';ctx.fillStyle=storageSelected?'rgba(214,255,63,.18)':'rgba(255,184,92,.06)';ctx.fillRect(755,450,155,38);ctx.strokeStyle=storageSelected?'#d6ff3f':'#80663d';ctx.lineWidth=storageSelected?3:1;ctx.strokeRect(755,450,155,38);ctx.fillStyle='#ffb85c';ctx.font='700 12px Space Mono';ctx.fillText(storage.label,765,465);ctx.fillStyle='#8a795e';ctx.fillText('REMOVE MODIFIER',765,480);
    const tooltip=game.equipmentTooltip();ctx.fillStyle='rgba(4,10,14,.94)';ctx.fillRect(535,515,600,102);ctx.strokeStyle=active?'#d6ff3f':'#75f5ff';ctx.lineWidth=2;ctx.strokeRect(535,515,600,102);this.drawItemIcon(game.selectedInventoryItem(),550,531,38);ctx.fillStyle='#ffffff';ctx.font='700 13px Space Mono';ctx.fillText(tooltip.name,600,540);ctx.fillStyle='#aab9bc';ctx.font='13px Space Mono';ctx.fillText(tooltip.detail,600,562);ctx.fillStyle=active?'#d6ff3f':'#75f5ff';ctx.font='700 13px Space Mono';ctx.fillText(tooltip.prompt,600,590);ctx.fillStyle='#71838a';ctx.font='12px Space Mono';ctx.fillText(`${items.length} ACQUIRED // ${game.player.body.internalSlots.length} INTERNAL BAYS`,140,615);if(rows.length>visibleCount){ctx.textAlign='right';ctx.fillText(`${game.inventorySelection+1} / ${rows.length}`,505,615);ctx.textAlign='left';}
  }
