...
export class Renderer {
  ...
  draw(game) {
    ...
    if(game.merchantMenuOpen)this.drawMerchantMenu(game);
    if(game.shellMenuOpen)this.drawShellMenu(game);
    ctx.restore();
  }
  ...
  drawShellPreview(shellId,x,y,scale=1) {
    const {ctx}=this;ctx.save();ctx.translate(x,y);ctx.scale(scale,scale);
    if(shellId==='slicer-body'){
      ctx.fillStyle='#1a242b';ctx.beginPath();ctx.moveTo(29,0);ctx.bezierCurveTo(11,-20,-12,-20,-25,-7);ctx.bezierCurveTo(-31,0,-25,12,-12,17);ctx.bezierCurveTo(5,22,19,12,29,0);ctx.fill();ctx.strokeStyle='#ffb85c';ctx.lineWidth=3;ctx.stroke();ctx.fillStyle='#2b3940';ctx.beginPath();ctx.ellipse(-8,-2,15,10,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#dce6dd';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(14,-12);ctx.lineTo(34,-17);ctx.moveTo(16,11);ctx.lineTo(35,16);ctx.stroke();ctx.fillStyle='#ff3d3d';for(const [eyeX,eyeY,radius] of [[2,-6,4],[10,0,3],[1,6,2.5]]){ctx.beginPath();ctx.arc(eyeX,eyeY,radius,0,Math.PI*2);ctx.fill();}
    }else{
      ctx.fillStyle='#1a242b';ctx.beginPath();ctx.ellipse(0,0,25,18,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#93a2a5';ctx.lineWidth=3;ctx.stroke();ctx.fillStyle='#2b3940';ctx.beginPath();ctx.ellipse(-5,-4,17,11,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#ff3d3d';for(const [eyeX,eyeY,radius] of [[12,-6,4],[19,0,2.5],[10,5,2]]){ctx.beginPath();ctx.arc(eyeX,eyeY,radius,0,Math.PI*2);ctx.fill();}ctx.fillStyle='#d6ff3f';ctx.fillRect(-8,-21,16,3);
    }
    ctx.restore();
  }
  ...
  drawBot(game) {
    ...
    if(player.body.id==='slicer-body'){
      ctx.save();ctx.translate(player.x+player.w/2,player.y+player.h/2);ctx.scale(player.facing,1);ctx.shadowBlur=8;ctx.shadowColor='#ffb85c';ctx.fillStyle='#1a242b';ctx.beginPath();ctx.moveTo(29,0);ctx.bezierCurveTo(11,-20,-12,-20,-25,-7);ctx.bezierCurveTo(-31,0,-25,12,-12,17);ctx.bezierCurveTo(5,22,19,12,29,0);ctx.fill();ctx.shadowBlur=0;ctx.strokeStyle='#ffb85c';ctx.lineWidth=3;ctx.stroke();ctx.fillStyle='#2b3940';ctx.beginPath();ctx.ellipse(-8,-2,15,10,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#dce6dd';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(14,-12);ctx.lineTo(34,-17);ctx.moveTo(16,11);ctx.lineTo(35,16);ctx.stroke();ctx.shadowBlur=10;ctx.shadowColor='#ff3434';ctx.fillStyle='#ff3d3d';for(const [eyeX,eyeY,radius] of [[2,-6,4],[10,0,3],[1,6,2.5]]){ctx.beginPath();ctx.arc(eyeX,eyeY,radius,0,Math.PI*2);ctx.fill();}ctx.shadowBlur=0;ctx.restore();
    }else{
  ...
  drawPickup(pickup,time) {
    const {ctx}=this,cx=pickup.x+pickup.w/2,cy=pickup.y+pickup.h/2+Math.sin(time*4+pickup.x)*5,color=pickup.color??'#ffffff';ctx.save();ctx.translate(cx,cy);
    if(pickup.kind==='shell'){
      ctx.shadowBlur=24;ctx.shadowColor=color;ctx.fillStyle='#1a242b';ctx.beginPath();ctx.moveTo(18,0);ctx.bezierCurveTo(7,-13,-8,-13,-17,-5);ctx.bezierCurveTo(-21,0,-17,8,-8,11);ctx.bezierCurveTo(3,14,12,8,18,0);ctx.fill();ctx.strokeStyle=color;ctx.lineWidth=2;ctx.stroke();ctx.shadowBlur=0;ctx.fillStyle='#ff3d3d';for(const [x,y,r] of [[1,-4,2.5],[7,0,2],[1,4,1.7]]){ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();}ctx.strokeStyle='#ffffff';ctx.globalAlpha=.7;ctx.beginPath();ctx.arc(0,0,27,time,time+Math.PI*1.35);ctx.stroke();ctx.restore();return;
    }
    ctx.rotate(time*.8);ctx.shadowBlur=24;ctx.shadowColor=color;ctx.fillStyle=color;ctx.fillRect(-pickup.w/2,-pickup.h/2,pickup.w,pickup.h);ctx.shadowBlur=0;ctx.strokeStyle='#ffffff';ctx.lineWidth=2;ctx.strokeRect(-pickup.w/2-3,-pickup.h/2-3,pickup.w+6,pickup.h+6);ctx.rotate(-time*1.7);ctx.globalAlpha=.75;ctx.beginPath();ctx.arc(0,0,24,0,Math.PI*1.3);ctx.stroke();ctx.beginPath();ctx.arc(0,0,34,Math.PI,Math.PI*2.3);ctx.stroke();for(let i=0;i<4;i++){const angle=time*2+i*Math.PI/2;ctx.fillStyle=i%2?'#ffffff':color;ctx.fillRect(Math.cos(angle)*30-2,Math.sin(angle)*30-2,4,4);}ctx.restore();
  }
  ...
  drawCrownBossArena(game) {
    const {ctx}=this,boss=game.crownBoss();
    for(const gate of game.crownBossGates()){ctx.save();ctx.fillStyle='#10262d';ctx.fillRect(gate.x,gate.y,gate.w,gate.h);ctx.strokeStyle='#75f5ff';ctx.lineWidth=3;ctx.strokeRect(gate.x+2,gate.y+2,gate.w-4,gate.h-4);for(let x=gate.x+8;x<gate.x+gate.w-6;x+=18){ctx.fillStyle='#d6ff3f';ctx.fillRect(x,gate.y+gate.h/2-2,5,4);}ctx.restore();}
    for(const hazard of game.crownHazards){const color=hazard.type==='column'?'#ffffff':'#75f5ff';ctx.save();ctx.globalAlpha=.55+.2*Math.sin(game.time*30);ctx.shadowBlur=24;ctx.shadowColor=color;ctx.fillStyle=hazard.type==='column'?'rgba(255,255,255,.28)':'rgba(117,245,255,.35)';ctx.fillRect(hazard.x,hazard.y,hazard.w,hazard.h);ctx.strokeStyle=color;ctx.lineWidth=4;ctx.strokeRect(hazard.x,hazard.y,hazard.w,hazard.h);ctx.restore();}
    if(boss?.bossMove==='crownColumnWindup'){ctx.save();ctx.globalAlpha=.55+.25*Math.sin(game.time*24);ctx.strokeStyle='#ffffff';ctx.lineWidth=3;ctx.setLineDash([10,10]);ctx.beginPath();ctx.moveTo(boss.crownTargetX,game.crownBossArena.y);ctx.lineTo(boss.crownTargetX,game.crownBossArena.floorY);ctx.stroke();ctx.restore();}
    if(boss?.bossMove==='crownSweepWindup'){ctx.save();ctx.globalAlpha=.55+.25*Math.sin(game.time*24);ctx.strokeStyle='#ffffff';ctx.lineWidth=3;ctx.setLineDash([10,10]);ctx.beginPath();ctx.moveTo(game.crownBossArena.x,game.crownBossArena.floorY-30);ctx.lineTo(game.crownBossArena.x+game.crownBossArena.w,game.crownBossArena.floorY-30);ctx.stroke();ctx.restore();}
    if(boss?.bossMove==='crownGridWindup'){ctx.save();ctx.globalAlpha=.55+.25*Math.sin(game.time*24);ctx.strokeStyle='#ffffff';ctx.lineWidth=3;ctx.setLineDash([10,10]);for(const ratio of[.32,.68]){const x=game.crownBossArena.x+game.crownBossArena.w*ratio;ctx.beginPath();ctx.moveTo(x,game.crownBossArena.y);ctx.lineTo(x,game.crownBossArena.floorY);ctx.stroke();}ctx.restore();}
  }

  drawGauntletHazards(game) {
    const {ctx}=this;
    for(const hazard of game.gauntletHazards){
      if(hazard.type==='electric'){ctx.save();ctx.fillStyle='#28171a';ctx.fillRect(hazard.x-7,hazard.y,hazard.w+14,12);ctx.fillRect(hazard.x-7,hazard.y+hazard.h-12,hazard.w+14,12);ctx.strokeStyle=hazard.active?'#75f5ff':'#39494e';ctx.lineWidth=hazard.active?5:2;ctx.shadowBlur=hazard.active?22:0;ctx.shadowColor='#75f5ff';ctx.beginPath();ctx.moveTo(hazard.x+hazard.w/2,hazard.y+12);ctx.lineTo(hazard.x+hazard.w/2,hazard.y+hazard.h-12);ctx.stroke();if(hazard.active){ctx.globalAlpha=.35;ctx.lineWidth=14;ctx.stroke();}ctx.restore();continue;}
      const ball=game.gauntletSwingPosition(hazard);ctx.save();ctx.strokeStyle='#69767b';ctx.lineWidth=7;ctx.beginPath();ctx.moveTo(hazard.anchorX,hazard.anchorY);ctx.lineTo(ball.x,ball.y);ctx.stroke();for(let step=22;step<hazard.length;step+=24){const ratio=step/hazard.length,x=hazard.anchorX+(ball.x-hazard.anchorX)*ratio,y=hazard.anchorY+(ball.y-hazard.anchorY)*ratio;ctx.fillStyle='#192228';ctx.strokeStyle='#849196';ctx.lineWidth=2;ctx.beginPath();ctx.arc(x,y,7,0,Math.PI*2);ctx.fill();ctx.stroke();}ctx.translate(ball.x,ball.y);ctx.fillStyle='#29191b';ctx.strokeStyle='#ff493f';ctx.lineWidth=5;ctx.beginPath();ctx.arc(0,0,hazard.radius,0,Math.PI*2);ctx.fill();ctx.stroke();for(let spike=0;spike<10;spike++){const angle=spike*Math.PI/5;ctx.rotate(angle);this.polygon([[hazard.radius-2,-6],[hazard.radius+15,0],[hazard.radius-2,6]],'#ff493f');ctx.rotate(-angle);}ctx.restore();
    }
  }
  ...
  drawJunkPile(pile) {
    const {ctx}=this,damage=1-pile.health/pile.maxHealth,materialTypes=pile.materials?.map(item=>item.type)??(pile.material?[pile.material.type]:[]),materialColor=materialTypes.includes('uranium')?'#83ff58':materialTypes.includes('titanium')?'#d6e2e5':null;ctx.save();ctx.translate(pile.x,pile.y);ctx.fillStyle=materialColor?'#293438':'#3a3029';ctx.beginPath();ctx.moveTo(0,pile.h);ctx.lineTo(6,pile.h*.52);ctx.lineTo(pile.w*.25,pile.h*.25);ctx.lineTo(pile.w*.48,pile.h*.38);ctx.lineTo(pile.w*.68,3);ctx.lineTo(pile.w*.84,pile.h*.3);ctx.lineTo(pile.w,pile.h*.58);ctx.lineTo(pile.w,pile.h);ctx.closePath();ctx.fill();ctx.strokeStyle=materialColor??'#87705b';ctx.lineWidth=3;ctx.stroke();
    ...
    if(materialTypes.length){ctx.shadowBlur=12;ctx.shadowColor=materialColor;ctx.fillStyle=materialColor;ctx.font='700 12px Space Mono';ctx.fillText(materialTypes.length>1?'TI+U':materialTypes[0]==='uranium'?'U':'TI',8,13);ctx.shadowBlur=0;}if(pile.minimumDamage>1||pile.requires){ctx.strokeStyle='#75f5ff';ctx.lineWidth=3;ctx.strokeRect(3,3,pile.w-6,pile.h-6);ctx.fillStyle='#75f5ff';ctx.font='700 13px Space Mono';ctx.fillText(pile.requires==='field'?'FIELD':'VOLT',pile.w/2-(pile.requires==='field'?16:13),pile.h/2+3);}if(damage>0){ctx.strokeStyle='#d6ff3f';ctx.globalAlpha=.35+damage*.5;ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(pile.w*.52,4);ctx.lineTo(pile.w*.45,pile.h*.42);ctx.lineTo(pile.w*.58,pile.h*.7);ctx.stroke();}ctx.restore();
  }
  ...
  drawEnemy(enemy,time) {
    ...
    if(!enemy.isCrownBoss)this.drawEnemyAppendages(enemy);
    ...
    if(enemy.isDepthBoss){const hovering=enemy.bossMove==='stalkerDropHover',warning=enemy.bossMove?.endsWith('Windup')||hovering,dashing=enemy.bossMove==='stalkerDash',dropping=enemy.bossMove==='stalkerDrop'||hovering;ctx.rotate(dashing?Math.sign(enemy.vx)*.06:0);this.polygon([[-enemy.w/2+8,enemy.h/2-8],[-enemy.w/2,-enemy.h/2+15],[-enemy.w*.2,-enemy.h/2-8],[enemy.w*.28,-enemy.h/2],[enemy.w/2,-enemy.h/2+18],[enemy.w/2-4,enemy.h/2-8],[20,enemy.h/2],[-22,enemy.h/2]],dashing?'#40502c':dropping?'#354132':'#1e2c26');ctx.strokeStyle=warning?'#ffffff':'#d6ff3f';ctx.lineWidth=5;ctx.strokeRect(-enemy.w/2+11,-enemy.h/2+11,enemy.w-22,enemy.h-24);ctx.fillStyle='#08110c';ctx.fillRect(-27,-13,54,26);ctx.shadowBlur=warning?28:16;ctx.shadowColor=warning?'#ffffff':'#d6ff3f';ctx.fillStyle=warning?'#ffffff':'#d6ff3f';for(const eyeX of[-15,0,15]){ctx.beginPath();ctx.arc(eyeX,0,5,0,Math.PI*2);ctx.fill();}ctx.shadowBlur=0;for(const side of[-1,1]){this.line(side*enemy.w*.35,-enemy.h*.18,side*enemy.w*.62,-enemy.h*.42,'#aeb9b1',5);this.polygon([[side*enemy.w*.62,-enemy.h*.42],[side*enemy.w*.5,-enemy.h*.48],[side*enemy.w*.57,-enemy.h*.27]],'#dce6dd');}ctx.restore();return;}
    if(enemy.isCrownBoss){const hovering=enemy.bossMove==='crownExpose',warning=enemy.bossMove?.endsWith('Windup'),relocating=enemy.bossMove==='crownRelocate';ctx.rotate(Math.sin(time*3)*.025);this.polygon([[-enemy.w/2+8,0],[-enemy.w*.34,-enemy.h/2],[0,-enemy.h*.36],[enemy.w*.34,-enemy.h/2],[enemy.w/2-8,0],[enemy.w*.32,enemy.h/2],[0,enemy.h*.34],[-enemy.w*.32,enemy.h/2]],relocating?'#18343c':'#1d2b31');ctx.strokeStyle=hovering?'#ffffff':warning?'#d6ff3f':'#75f5ff';ctx.lineWidth=5;ctx.strokeRect(-enemy.w/2+14,-enemy.h/2+10,enemy.w-28,enemy.h-20);ctx.fillStyle='#071116';ctx.fillRect(-29,-14,58,28);ctx.shadowBlur=hovering?30:warning?22:14;ctx.shadowColor=hovering?'#ffffff':warning?'#d6ff3f':'#75f5ff';ctx.fillStyle=hovering?'#ffffff':warning?'#d6ff3f':'#75f5ff';for(const x of[-17,0,17]){ctx.beginPath();ctx.arc(x,0,5,0,Math.PI*2);ctx.fill();}ctx.shadowBlur=0;for(const side of[-1,1]){this.line(side*enemy.w*.38,-4,side*enemy.w*.65,-18,'#8da1a6',5);this.polygon([[side*enemy.w*.65,-18],[side*enemy.w*.56,-29],[side*enemy.w*.52,-10]],'#d5e2e4');}ctx.restore();return;}
  ...
  drawInventory(game) {
    ...
    const instructions=page==='map'?(game.mapOverview?'BOT SYSTEM ARCHIVE // I TO CLOSE // W A S D SELECT // Q OPEN MAP':'BOT SYSTEM ARCHIVE // I TO CLOSE // Q WORLD OVERVIEW // D STATUS'):page==='items'?(game.equipmentItemIndex===null?'LOADOUT // I CLOSE // A D PAGES // W S ITEMS // Q SELECT':'PLACEMENT // ARROWS CHOOSE SLOT // Q INSTALL // I CLOSE'):'BOT SYSTEM ARCHIVE // I TO CLOSE // A D PAGES // W S ENTRIES';
    ctx.fillStyle='#71838a';ctx.font='700 12px Space Mono';ctx.fillText(instructions,140,105);
    ...
    if(page==='map')this.drawInventoryMap(game,ctx);else if(page==='items')this.drawEquipmentInventory(game,ctx);else{
      const titles={status:'CORE STATUS',materials:'SPECIAL MATERIALS'},rows=page==='status'?[['CORE SHELLS',game.godMode?'INFINITE':`${game.player.lives} / ${game.player.maxLives}`],['ELECTRICITY',game.godMode?'INFINITE':`${Math.round(game.player.electricity)} / ${game.player.maxElectricity}`],['FUSION CUTTER',`${game.player.primaryDamage} DMG // ${game.player.primaryRange} RANGE`],['SCRAP',String(game.player.scrap)]]:[['TITANIUM',String(game.player.materials.titanium)],['URANIUM',String(game.player.materials.uranium)]],visibleCount=6,start=0;
      ctx.fillStyle='#ffffff';ctx.font='700 24px Space Mono';ctx.fillText(titles[page],150,215);
      rows.slice(start,start+visibleCount).forEach((row,index)=>{const actualIndex=start+index,ry=250+index*56,selected=actualIndex===game.inventorySelection;ctx.fillStyle=selected?'rgba(117,245,255,.12)':'rgba(255,255,255,.025)';ctx.fillRect(150,ry,980,44);ctx.strokeStyle=selected?'#75f5ff':'#27363c';ctx.strokeRect(150,ry,980,44);ctx.fillStyle=selected?'#ffffff':'#84959a';ctx.font='700 12px Space Mono';ctx.fillText(row[0],172,ry+18);ctx.textAlign='right';ctx.fillStyle=selected?'#d6ff3f':'#a9b6b9';ctx.font='700 12px Space Mono';ctx.fillText(row[1],1105,ry+27);ctx.textAlign='left';});
    }
    ctx.restore();
  }

  drawItemIcon(item,x,y,size=30) {
    const {ctx}=this,color=item?.kind==='relic'?'#d6ff3f':item?.kind==='modifier'?'#75f5ff':'#ffb85c';ctx.save();ctx.translate(x+size/2,y+size/2);ctx.strokeStyle=color;ctx.fillStyle='rgba(117,245,255,.08)';ctx.lineWidth=2;
    if(item?.modifierId==='aegis-filament'){ctx.beginPath();ctx.moveTo(0,-size*.38);ctx.lineTo(size*.32,-size*.18);ctx.lineTo(size*.23,size*.28);ctx.lineTo(0,size*.42);ctx.lineTo(-size*.23,size*.28);ctx.lineTo(-size*.32,-size*.18);ctx.closePath();ctx.fill();ctx.stroke();this.line(-size*.13,0,size*.13,0,color,2);}
    else if(item?.modifierId==='reactive-governor'){ctx.beginPath();ctx.moveTo(-size*.12,-size*.4);ctx.lineTo(size*.18,-size*.08);ctx.lineTo(0,-size*.08);ctx.lineTo(size*.12,size*.4);ctx.lineTo(-size*.2,size*.08);ctx.lineTo(0,size*.08);ctx.closePath();ctx.stroke();}
    else if(item?.modifierId==='extender-arm'){this.line(-size*.35,size*.25,-size*.08,-size*.1,color,4);this.line(-size*.08,-size*.1,size*.32,-size*.28,color,4);ctx.strokeRect(size*.2,-size*.4,size*.18,size*.2);}
    else if(item?.kind==='relic'){ctx.rotate(Math.PI/4);ctx.fillStyle='rgba(214,255,63,.12)';ctx.fillRect(-size*.25,-size*.25,size*.5,size*.5);ctx.strokeRect(-size*.25,-size*.25,size*.5,size*.5);ctx.rotate(-Math.PI/4);ctx.beginPath();ctx.arc(0,0,size*.1,0,Math.PI*2);ctx.fillStyle=color;ctx.fill();}
    else{for(let i=0;i<3;i++){ctx.fillStyle=i===2?color:'#33454c';ctx.fillRect(-size*.34,-size*.3+i*size*.24,size*.68,size*.12);}}
    ctx.restore();
  }

  drawEquipmentInventory(game,ctx) {
    const items=game.player.purchasedItems,rows=items.length?items:[{name:'NO ACQUIRED ITEMS',kind:'empty',detail:'MERCHANT EQUIPMENT WILL APPEAR HERE'}],visibleCount=7,start=Math.max(0,Math.min(game.inventorySelection-visibleCount+1,rows.length-visibleCount)),listX=140,listY=220,listW=365,rowH=48;
    ctx.fillStyle='#ffffff';ctx.font='700 20px Space Mono';ctx.fillText('ACQUIRED EQUIPMENT',listX,200);ctx.fillStyle='#71838a';ctx.font='12px Space Mono';ctx.fillText('MODIFIERS + RELICS SHARE NORMAL MOUNTS',listX,214);
    rows.slice(start,start+visibleCount).forEach((item,index)=>{const actual=start+index,ry=listY+index*rowH,selected=actual===game.inventorySelection,slotted=['modifier','relic'].includes(item.kind);ctx.fillStyle=selected?'rgba(117,245,255,.13)':'rgba(255,255,255,.025)';ctx.fillRect(listX,ry,listW,rowH-5);ctx.strokeStyle=selected?'#75f5ff':'#27363c';ctx.lineWidth=selected?2:1;ctx.strokeRect(listX,ry,listW,rowH-5);this.drawItemIcon(item,listX+9,ry+7,28);ctx.fillStyle=selected?'#ffffff':'#8b9ba0';ctx.font='700 12px Space Mono';ctx.fillText(item.name,listX+48,ry+18);ctx.fillStyle=item.kind==='modifier'?'#75f5ff':item.kind==='relic'?'#d6ff3f':'#ffb85c';ctx.font='12px Space Mono';ctx.fillText(slotted?(item.equippedSlot?'INSTALLED':'STORAGE // INACTIVE'):'PERMANENT',listX+48,ry+34);});
    const panel={x:535,y:190,w:600,h:310};ctx.fillStyle='rgba(10,18,22,.78)';ctx.fillRect(panel.x,panel.y,panel.w,panel.h);ctx.strokeStyle='#33454c';ctx.strokeRect(panel.x,panel.y,panel.w,panel.h);ctx.fillStyle='#ffffff';ctx.font='700 18px Space Mono';ctx.fillText(game.player.body.name,panel.x+20,panel.y+28);ctx.fillStyle='#71838a';ctx.font='12px Space Mono';ctx.fillText('CURRENT SHELL // NORMAL + EXPANSION MOUNTS',panel.x+20,panel.y+46);
    const bodyX=830,bodyY=360;this.drawShellPreview(game.player.body.id,bodyX,bodyY,1.55);this.line(bodyX-38,bodyY+30,bodyX-66,bodyY+67,'#87969b',5);this.line(bodyX,bodyY+30,bodyX-8,bodyY+70,'#87969b',5);this.line(bodyX+38,bodyY+30,bodyX+66,bodyY+67,'#87969b',5);
    const chosen=game.equipmentItemIndex===null?null:game.player.purchasedItems[game.equipmentItemIndex],target=game.selectedEquipmentTarget(),active=Boolean(chosen),bodySlots=game.bodySlots(),expansionSlots=game.player.body.expansionSlots,displayExpansionSlots=game.baySlotDefinitions().map(definition=>expansionSlots.find(slot=>slot.id===definition.id)??{...definition,locked:true}),normalSlotIds=new Set(game.normalSlots().map(slot=>slot.id)),positions=game.player.body.id==='slicer-body'?{core:{x:555,y:300,w:150},legs:{x:555,y:380,w:150},weapon:{x:955,y:300,w:160},'extended-weapon':{x:935,y:380,w:180}}:{shell:{x:555,y:270,w:150},core:{x:555,y:330,w:150},legs:{x:555,y:390,w:150},weapon:{x:955,y:330,w:150}},slotBoxes=[...game.normalSlots().map(slot=>({slot,...positions[slot.id]})),...displayExpansionSlots.map((slot,index)=>({slot,x:720+index*130,y:250,w:120}))];
    const drawSlot=({slot,x,y,w=150})=>{if(!slot)return;const occupant=items.find(item=>item.equippedSlot===slot.id),compatible=!active||(!slot.locked&&((chosen.kind==='relic'&&normalSlotIds.has(slot.id))||(chosen.kind==='modifier'&&Boolean(game.modifierEffect(chosen,slot))))),selected=active&&target?.id===slot.id,color=occupant?.kind==='relic'||chosen?.kind==='relic'&&normalSlotIds.has(slot.id)?'#d6ff3f':slot.part==='internal'||slot.part==='auxiliary'?'#ffb85c':'#75f5ff';ctx.fillStyle=selected?'rgba(214,255,63,.18)':compatible?`${color}14`:'rgba(255,255,255,.02)';ctx.fillRect(x,y,w,42);ctx.strokeStyle=selected?'#ffffff':compatible?color:'#354047';ctx.lineWidth=selected?3:1;ctx.strokeRect(x,y,w,42);ctx.fillStyle=compatible?'#ffffff':'#4c5b60';ctx.font='700 12px Space Mono';ctx.fillText(slot.label.replace('INTERNAL','INT').replace('AUXILIARY','AUX'),x+7,y+15);ctx.fillStyle=occupant?'#d6ff3f':'#65757a';ctx.font='12px Space Mono';ctx.fillText(occupant?occupant.name.slice(0,15):slot.locked?'LOCKED':compatible?'EMPTY':'INCOMPATIBLE',x+7,y+32);};slotBoxes.forEach(drawSlot);
    const storageSelected=active&&target?.part==='storage';ctx.fillStyle=storageSelected?'rgba(214,255,63,.18)':'rgba(255,184,92,.06)';ctx.fillRect(555,452,150,36);ctx.strokeStyle=storageSelected?'#d6ff3f':'#80663d';ctx.lineWidth=storageSelected?3:1;ctx.strokeRect(555,452,150,36);ctx.fillStyle='#ffb85c';ctx.font='700 12px Space Mono';ctx.fillText('STORAGE',564,466);ctx.fillStyle='#8a795e';ctx.fillText('REMOVE ITEM',564,481);
    const tooltip=game.equipmentTooltip();ctx.fillStyle='rgba(4,10,14,.94)';ctx.fillRect(535,515,600,102);ctx.strokeStyle=active?'#d6ff3f':'#75f5ff';ctx.lineWidth=2;ctx.strokeRect(535,515,600,102);this.drawItemIcon(game.selectedInventoryItem(),550,531,38);ctx.fillStyle='#ffffff';ctx.font='700 13px Space Mono';ctx.fillText(tooltip.name,600,540);ctx.fillStyle='#aab9bc';ctx.font='13px Space Mono';ctx.fillText(tooltip.detail,600,562);ctx.fillStyle=active?'#d6ff3f':'#75f5ff';ctx.font='700 13px Space Mono';ctx.fillText(tooltip.prompt,600,590);ctx.fillStyle='#71838a';ctx.font='12px Space Mono';ctx.fillText(`${items.length} ACQUIRED // ${expansionSlots.length}/3 EXPANSION // 1 INTERNAL MAX`,140,615);if(rows.length>visibleCount){ctx.textAlign='right';ctx.fillText(`${game.inventorySelection+1} / ${rows.length}`,505,214);ctx.textAlign='left';}
  }

  drawMerchantMenu(game) {
    const {ctx}=this,merchant=game.merchantRoom.activeMerchant,rows=game.merchantCatalog(merchant),x=110,y=70,w=1060,h=580;ctx.save();ctx.fillStyle='rgba(2,7,11,.98)';ctx.fillRect(x,y,w,h);ctx.strokeStyle=merchant.color;ctx.lineWidth=3;ctx.strokeRect(x,y,w,h);ctx.fillStyle=merchant.color;ctx.fillRect(x,y,w,7);
    ctx.fillStyle='#71838a';ctx.font='700 12px Space Mono';ctx.fillText('MERCHANT CATALOG // I CLOSE // W S SELECT // O BUY',140,105);
    ctx.fillStyle='#ffffff';ctx.font='700 28px Space Mono';ctx.fillText(merchant.name,140,155);ctx.textAlign='right';ctx.fillStyle='#d6ff3f';ctx.font='700 16px Space Mono';ctx.fillText(`${game.player.scrap} SCRAP`,1135,150);ctx.textAlign='left';
    ctx.fillStyle='#65757a';ctx.font='13px Space Mono';ctx.fillText(merchant.specialty??'FULL INVENTORY // SELECT AN OFFER TO PURCHASE',140,180);
    rows.forEach((row,index)=>{const ry=205+index*82,selected=index===game.merchantMenuSelection,available=row.state==='available',materialsAffordable=game.canAffordMaterials(row.materials??{}),affordable=available&&game.player.scrap>=row.cost&&materialsAffordable;ctx.fillStyle=selected?'rgba(117,245,255,.13)':'rgba(255,255,255,.025)';ctx.fillRect(140,ry,995,68);ctx.strokeStyle=selected?merchant.color:'#27363c';ctx.lineWidth=selected?2:1;ctx.strokeRect(140,ry,995,68);ctx.fillStyle=row.state==='owned'?'#71838a':selected?'#ffffff':'#91a0a4';ctx.font='700 13px Space Mono';ctx.fillText(row.name,160,ry+24);ctx.textAlign='right';ctx.fillStyle=row.state==='owned'?'#71838a':affordable?'#d6ff3f':available?'#ff786f':'#59676c';ctx.font='700 12px Space Mono';const status=row.state==='owned'?'OWNED':row.state==='locked'?`${row.cost} SCRAP // BUY PREVIOUS TIER`:row.state==='unavailable'?'UNAVAILABLE':!materialsAffordable?`NEED ${game.materialCostText(row.materials)}`:affordable?`O BUY // ${row.cost} SCRAP`:`NEED ${row.cost} SCRAP`;ctx.fillText(status,1115,ry+24);ctx.textAlign='left';ctx.fillStyle=selected?'#aab9bc':'#65757a';ctx.font='12px Space Mono';ctx.fillText(row.detail,160,ry+50);});
    ctx.fillStyle='#71838a';ctx.font='13px Space Mono';ctx.fillText(`MATERIALS CARRIED // TITANIUM ${game.player.materials.titanium} // URANIUM ${game.player.materials.uranium}`,140,625);ctx.textAlign='right';ctx.fillText(`${game.merchantMenuSelection+1} / ${rows.length}`,1135,625);ctx.textAlign='left';ctx.restore();
  }

  drawShellMenu(game) {
    const {ctx}=this,shells=game.player.ownedShells,x=110,y=70,w=1060,h=580;ctx.save();ctx.fillStyle='rgba(2,7,11,.98)';ctx.fillRect(x,y,w,h);ctx.strokeStyle='#ffb85c';ctx.lineWidth=3;ctx.strokeRect(x,y,w,h);ctx.fillStyle='#ffb85c';ctx.fillRect(x,y,w,7);ctx.fillStyle='#71838a';ctx.font='700 12px Space Mono';ctx.fillText('RECOVERY STATION // ARROWS SELECT // O EQUIP // I KEEP CURRENT',140,105);ctx.fillStyle='#ffffff';ctx.font='700 28px Space Mono';ctx.fillText('SELECT ACTIVE SHELL',140,150);ctx.fillStyle='#71838a';ctx.font='13px Space Mono';ctx.fillText('EACH SHELL RESTORES ITS OWN SAVED EQUIPMENT LOADOUT',140,178);
    const cardW=440,gap=30,startX=180,cardY=215;for(let index=0;index<shells.length;index++){const id=shells[index],definition=game.shellDefinition(id),selected=index===game.shellMenuSelection,current=id===game.player.body.id,cx=startX+index*(cardW+gap);ctx.fillStyle=selected?'rgba(255,184,92,.12)':'rgba(255,255,255,.025)';ctx.fillRect(cx,cardY,cardW,330);ctx.strokeStyle=selected?'#ffb85c':'#33454c';ctx.lineWidth=selected?3:1;ctx.strokeRect(cx,cardY,cardW,330);ctx.fillStyle='#ffffff';ctx.font='700 20px Space Mono';ctx.fillText(definition.name,cx+24,cardY+38);ctx.textAlign='right';ctx.fillStyle=current?'#d6ff3f':'#65757a';ctx.font='700 12px Space Mono';ctx.fillText(current?'EQUIPPED':'AVAILABLE',cx+cardW-22,cardY+36);ctx.textAlign='left';this.drawShellPreview(id,cx+cardW/2,cardY+112,2.15);ctx.fillStyle='#8da0a6';ctx.font='12px Space Mono';ctx.fillText(definition.description,cx+24,cardY+176);ctx.fillStyle='#ffffff';ctx.font='700 13px Space Mono';ctx.fillText(`CORE SHELLS  ${definition.baseLives+game.player.healthUpgrades}`,cx+24,cardY+216);ctx.fillText(`ELECTRICITY  ${definition.baseElectricity+game.player.energyUpgrades*25}`,cx+24,cardY+244);ctx.fillText(`CUTTER RANGE ${definition.baseRange}`,cx+24,cardY+272);ctx.fillStyle='#71838a';ctx.font='12px Space Mono';ctx.fillText(definition.slots.some(slot=>slot.part==='shell')?'SHELL MOUNT + CORE + LEGS + CUTTER':'NO SHELL MOUNT // CORE + LEGS + TWO CUTTERS',cx+24,cardY+304);}
    ctx.fillStyle='#71838a';ctx.font='13px Space Mono';ctx.fillText('REST COMPLETE // SELECTED SHELL WILL BE FULLY REPAIRED',140,620);ctx.restore();
  }
  ...
  drawMapOverview(game,ctx) {
    ...
    ctx.fillStyle='#71838a';ctx.font='12px Space Mono';ctx.fillText('W A S D SELECT REGION // Q ZOOM IN',x,y+25);
    for(let index=0;index<game.regions.length;index++){
      ...
      ctx.fillStyle=known?'#71858c':'#39464a';ctx.font='13px Space Mono';ctx.fillText(`REGION ${String(index+1).padStart(2,'0')}`,rx+18,ry+60);
      ...
      if(active){ctx.fillStyle='#d6ff3f';ctx.font='700 13px Space Mono';ctx.fillText('SELECTED',rx+18,ry+88);}
    }
  }
  ...
  drawLocalMap(game,ctx) {
    ...
    ctx.fillStyle='#71838a';ctx.font='12px Space Mono';ctx.fillText('LOCAL MAP // Q WORLD OVERVIEW',x,y+25);
    ...
    if(!known){ctx.textAlign='center';ctx.fillStyle='#3d4b50';ctx.font='700 44px Space Mono';ctx.fillText('NO MAP',x+w/2,y+225);ctx.fillStyle='#65757a';ctx.font='12px Space Mono';ctx.fillText('FIND A SURVEY CORE TO RESTORE THIS REGION',x+w/2,y+258);ctx.textAlign='left';return;}
    ...
    ctx.restore();ctx.fillStyle='#84959a';ctx.font='13px Space Mono';ctx.fillText('TRUE SCALE // FOUNDATIONS // ROUTES // HAZARDS // MERCHANT LINKS',x+24,y+h+36);
  }

  drawHud(game) {
    const {ctx}=this;ctx.fillStyle='rgba(7,11,16,.86)';ctx.fillRect(24,22,340,88);ctx.strokeStyle='#2f3b43';ctx.strokeRect(24,22,340,88);ctx.fillStyle='#75848a';ctx.font='13px Space Mono';ctx.fillText('CORE SHELLS',42,43);ctx.fillText('ELECTRICITY',42,72);ctx.fillText('SCRAP',255,43);
    for(let i=0;i<game.player.maxLives;i++){ctx.fillStyle=i<game.player.lives?'#ff493f':'#2d363b';ctx.beginPath();ctx.arc(140+i*13,40,4.5,0,Math.PI*2);ctx.fill();}if(game.godMode){ctx.fillStyle='#75f5ff';ctx.font='700 16px Space Mono';ctx.fillText('∞',220,45);}
    ctx.fillStyle='#263239';ctx.fillRect(140,67,190,7);ctx.fillStyle='#75f5ff';ctx.fillRect(140,67,game.godMode?190:190*(game.player.electricity/game.player.maxElectricity),7);ctx.fillStyle='#aab8ba';ctx.fillText(game.godMode?'∞ / ∞':`${Math.round(game.player.electricity)} / ${game.player.maxElectricity}`,140,92);ctx.fillStyle='#d6ff3f';ctx.font='700 16px Space Mono';ctx.fillText(String(game.player.scrap).padStart(3,'0'),302,45);
    ctx.textAlign='right';ctx.fillStyle='#708086';ctx.font='13px Space Mono';ctx.fillText(`X ${Math.round(game.player.x).toString().padStart(4,'0')}  Y ${Math.round(game.player.y).toString().padStart(4,'0')}`,1238,42);ctx.fillStyle=game.godMode?'#75f5ff':'#d6ff3f';ctx.fillText(game.godMode?'GOD MODE // ONLINE':'LOCAL GRID // OPEN',1238,60);ctx.fillStyle='#657278';ctx.fillText(`CORE KIT // CUTTER ${game.player.primaryDamage} DMG / ${game.player.primaryRange} RANGE`,1238,82);ctx.textAlign='left';
    const boss=game.boss();if(game.bossArena.active&&boss&&!boss.dead){const width=480,ratio=Math.max(0,boss.health/boss.maxHealth),x=(VIEW_WIDTH-width)/2,y=25;ctx.fillStyle='rgba(7,11,16,.92)';ctx.fillRect(x-12,y-12,width+24,42);ctx.strokeStyle=boss.phase===2?'#ffffff':'#5a2e31';ctx.lineWidth=2;ctx.strokeRect(x-12,y-12,width+24,42);ctx.fillStyle='#242c32';ctx.fillRect(x,y,width,12);ctx.fillStyle=boss.phase===2?'#ffffff':'#ff493f';ctx.fillRect(x,y,width*ratio,12);ctx.fillStyle='#d4dcde';ctx.font='700 12px Space Mono';ctx.textAlign='center';ctx.fillText(`HEAVY CORE${boss.phase===2?' // PHASE 2':''}`,VIEW_WIDTH/2,y+27);ctx.textAlign='left';}
    const vaultBoss=game.vaultBoss();if(game.vaultBossArena.active&&vaultBoss&&!vaultBoss.dead){const width=480,ratio=Math.max(0,vaultBoss.health/vaultBoss.maxHealth),x=(VIEW_WIDTH-width)/2,y=25;ctx.fillStyle='rgba(2,10,14,.94)';ctx.fillRect(x-12,y-12,width+24,42);ctx.strokeStyle=vaultBoss.phase===2?'#ffffff':'#315d68';ctx.lineWidth=2;ctx.strokeRect(x-12,y-12,width+24,42);ctx.fillStyle='#15242b';ctx.fillRect(x,y,width,12);ctx.fillStyle=vaultBoss.phase===2?'#ffffff':'#75f5ff';ctx.fillRect(x,y,width*ratio,12);ctx.fillStyle='#d8faff';ctx.font='700 12px Space Mono';ctx.textAlign='center';ctx.fillText(`${game.vaultBossArena.name}${vaultBoss.phase===2?' // PHASE 2':''}`,VIEW_WIDTH/2,y+27);ctx.textAlign='left';}
    const depthBoss=game.depthBoss();if(game.depthBossArena.active&&depthBoss&&!depthBoss.dead){const width=480,ratio=Math.max(0,depthBoss.health/depthBoss.maxHealth),x=(VIEW_WIDTH-width)/2,y=25;ctx.fillStyle='rgba(6,12,7,.94)';ctx.fillRect(x-12,y-12,width+24,42);ctx.strokeStyle=depthBoss.phase===2?'#ffffff':'#60752d';ctx.lineWidth=2;ctx.strokeRect(x-12,y-12,width+24,42);ctx.fillStyle='#20281a';ctx.fillRect(x,y,width,12);ctx.fillStyle=depthBoss.phase===2?'#ffffff':'#d6ff3f';ctx.fillRect(x,y,width*ratio,12);ctx.fillStyle='#f4ffd2';ctx.font='700 12px Space Mono';ctx.textAlign='center';ctx.fillText(`${game.depthBossArena.name}${depthBoss.phase===2?' // PHASE 2':''}`,VIEW_WIDTH/2,y+27);ctx.textAlign='left';}
    const crownBoss=game.crownBoss();if(game.crownBossArena.active&&crownBoss&&!crownBoss.dead){const width=480,ratio=Math.max(0,crownBoss.health/crownBoss.maxHealth),x=(VIEW_WIDTH-width)/2,y=25;ctx.fillStyle='rgba(3,12,16,.94)';ctx.fillRect(x-12,y-12,width+24,42);ctx.strokeStyle=crownBoss.phase===2?'#ffffff':'#4d7b85';ctx.lineWidth=2;ctx.strokeRect(x-12,y-12,width+24,42);ctx.fillStyle='#14262c';ctx.fillRect(x,y,width,12);ctx.fillStyle=crownBoss.phase===2?'#ffffff':'#75f5ff';ctx.fillRect(x,y,width*ratio,12);ctx.fillStyle='#e8fdff';ctx.font='700 12px Space Mono';ctx.textAlign='center';ctx.fillText(`${game.crownBossArena.name}${crownBoss.phase===2?' // PHASE 2':''}`,VIEW_WIDTH/2,y+27);ctx.textAlign='left';}
    const miniArena=game.miniBossArenas.find(arena=>arena.active&&!arena.cleared),miniBoss=miniArena?game.miniBoss(miniArena.id):null;if(miniBoss&&!miniBoss.dead){const width=360,ratio=Math.max(0,miniBoss.health/miniBoss.maxHealth),x=(VIEW_WIDTH-width)/2,y=26;ctx.fillStyle='rgba(4,10,15,.94)';ctx.fillRect(x-10,y-10,width+20,38);ctx.strokeStyle=miniBoss.phase===2?'#ffffff':'#315d68';ctx.strokeRect(x-10,y-10,width+20,38);ctx.fillStyle='#1a2a31';ctx.fillRect(x,y,width,9);ctx.fillStyle=miniBoss.phase===2?'#ffffff':'#75f5ff';ctx.fillRect(x,y,width*ratio,9);ctx.fillStyle='#d8faff';ctx.font='700 12px Space Mono';ctx.textAlign='center';ctx.fillText(`${miniArena.name}${miniBoss.phase===2?' // PHASE 2':''}`,VIEW_WIDTH/2,y+24);ctx.textAlign='left';}
    if(game.player.healTime>0){const progress=Math.round((1-game.player.healTime/.7)*100);ctx.fillStyle='rgba(7,11,16,.92)';ctx.fillRect(500,628,280,42);ctx.strokeStyle='#75f5ff';ctx.lineWidth=2;ctx.strokeRect(500,628,280,42);ctx.fillStyle='#75f5ff';ctx.font='700 13px Space Mono';ctx.textAlign='center';ctx.fillText(`REPAIRING // ${progress}%`,640,654);ctx.textAlign='left';}
    ...
    const door=game.nearbyMerchantDoor();if(door){const unlocked=game.merchantDoorUnlocked(door);ctx.fillStyle='rgba(7,11,16,.92)';ctx.fillRect(430,626,420,48);ctx.strokeStyle=unlocked?door.color:'#ff493f';ctx.lineWidth=2;ctx.strokeRect(430,626,420,48);ctx.fillStyle='#ffffff';ctx.font='700 13px Space Mono';ctx.textAlign='center';ctx.fillText(unlocked?`O  //  ENTER ${door.name}`:'ACCESS SEALED // LOCAL THREATS REMAIN',640,654);ctx.textAlign='left';}
    if(game.nearMerchantExit()){const merchant=game.merchantRoom.activeMerchant;ctx.fillStyle='rgba(7,11,16,.92)';ctx.fillRect(470,632,340,42);ctx.strokeStyle=merchant.color;ctx.strokeRect(470,632,340,42);ctx.fillStyle='#ffffff';ctx.font='700 13px Space Mono';ctx.textAlign='center';ctx.fillText('O  //  RETURN TO OVERWORLD',640,658);ctx.textAlign='left';}
    const merchant=game.nearbyMerchant();if(merchant){ctx.fillStyle='rgba(7,11,16,.92)';ctx.fillRect(410,626,460,48);ctx.strokeStyle=merchant.color;ctx.lineWidth=2;ctx.strokeRect(410,626,460,48);ctx.fillStyle='#ffffff';ctx.font='700 13px Space Mono';ctx.textAlign='center';ctx.fillText(`O  //  OPEN ${merchant.name} CATALOG`,640,654);ctx.textAlign='left';}
    if(game.regionToastTime>0&&game.regionToast){const alpha=Math.min(1,game.regionToastTime*2,(2.4-game.regionToastTime)*5);ctx.save();ctx.globalAlpha=Math.max(0,alpha);ctx.fillStyle='rgba(7,11,16,.92)';ctx.fillRect(24,608,360,58);ctx.strokeStyle='#75f5ff';ctx.lineWidth=2;ctx.strokeRect(24,608,360,58);ctx.fillStyle='#71838a';ctx.font='13px Space Mono';ctx.fillText('REGION LINK // NEW LOCAL GRID',42,630);ctx.fillStyle='#ffffff';ctx.font='700 17px Space Mono';ctx.fillText(game.regionToast,42,654);ctx.restore();}
    if(game.rewardToast?.time>0){const alpha=Math.min(1,game.rewardToast.time*2);ctx.save();ctx.globalAlpha=alpha;ctx.fillStyle='rgba(7,11,16,.94)';ctx.fillRect(480,116,320,64);ctx.strokeStyle='#d6ff3f';ctx.lineWidth=2;ctx.strokeRect(480,116,320,64);ctx.textAlign='center';ctx.fillStyle='#d6ff3f';ctx.font='700 20px Space Mono';ctx.fillText(game.rewardToast.text,640,143);ctx.fillStyle='#9caaae';ctx.font='13px Space Mono';ctx.fillText(game.rewardToast.detail,640,162);ctx.textAlign='left';ctx.restore();}
    if(game.abilityPopup?.time>0){const popup=game.abilityPopup,fade=Math.min(1,popup.time*2,(popup.maxTime-popup.time)*4);ctx.save();ctx.globalAlpha=Math.max(0,fade);ctx.fillStyle='rgba(2,7,12,.96)';ctx.fillRect(320,225,640,250);ctx.strokeStyle=popup.color;ctx.lineWidth=3;ctx.strokeRect(320,225,640,250);ctx.fillStyle=popup.color;ctx.fillRect(320,225,640,7);ctx.textAlign='center';ctx.fillStyle='#76888f';ctx.font='700 12px Space Mono';ctx.fillText('ABILITY CORE INTEGRATED',640,264);ctx.fillStyle='#ffffff';ctx.font='700 34px Space Mono';ctx.fillText(popup.name,640,310);ctx.fillStyle='#0b151b';ctx.fillRect(520,334,240,60);ctx.strokeStyle=popup.color;ctx.lineWidth=2;ctx.strokeRect(520,334,240,60);ctx.fillStyle=popup.color;ctx.font='700 25px Space Mono';ctx.fillText(popup.key,640,373);ctx.fillStyle='#afbec2';ctx.font='13px Space Mono';ctx.fillText(popup.description,640,425);if(popup.ability==='electricJab'){ctx.fillStyle='#72848a';ctx.font='13px Space Mono';ctx.fillText('Harvest the conduit ahead, then use F on the powered seal.',640,449);}if(popup.ability==='field'){ctx.fillStyle='#72848a';ctx.font='13px Space Mono';ctx.fillText('One charge supplied. Use Q beside the FIELD seal to open the annex.',640,449);}ctx.textAlign='left';ctx.restore();}
  }
}
