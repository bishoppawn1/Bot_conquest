...
export class Renderer {
  ...
  drawEquipmentInventory(game,ctx) {
    ...
    ctx.fillStyle='#ffffff';ctx.font='700 20px Space Mono';ctx.fillText('ACQUIRED EQUIPMENT',listX,200);ctx.fillStyle='#71838a';ctx.font='8px Space Mono';ctx.fillText('MODIFIERS INSTALL // RELICS STAY PASSIVE',listX,214);
    rows.slice(start,start+visibleCount).forEach((item,index)=>{const actual=start+index,ry=listY+index*rowH,selected=actual===game.inventorySelection;ctx.fillStyle=selected?'rgba(117,245,255,.13)':'rgba(255,255,255,.025)';ctx.fillRect(listX,ry,listW,rowH-5);ctx.strokeStyle=selected?'#75f5ff':'#27363c';ctx.lineWidth=selected?2:1;ctx.strokeRect(listX,ry,listW,rowH-5);this.drawItemIcon(item,listX+9,ry+7,28);ctx.fillStyle=selected?'#ffffff':'#8b9ba0';ctx.font='700 10px Space Mono';ctx.fillText(item.name,listX+48,ry+18);ctx.fillStyle=item.kind==='modifier'?'#75f5ff':item.kind==='relic'?'#d6ff3f':'#ffb85c';ctx.font='8px Space Mono';ctx.fillText(item.kind==='modifier'?(item.equippedSlot?'INSTALLED':'MODIFIER // STORAGE'):item.kind==='relic'?'PASSIVE RELIC':'PERMANENT',listX+48,ry+34);});
    const panel={x:535,y:190,w:600,h:310};ctx.fillStyle='rgba(10,18,22,.78)';ctx.fillRect(panel.x,panel.y,panel.w,panel.h);ctx.strokeStyle='#33454c';ctx.strokeRect(panel.x,panel.y,panel.w,panel.h);ctx.fillStyle='#ffffff';ctx.font='700 18px Space Mono';ctx.fillText(game.player.body.name,panel.x+20,panel.y+28);ctx.fillStyle='#71838a';ctx.font='8px Space Mono';ctx.fillText('CURRENT SHELL // LIVE MODIFIER MOUNTS',panel.x+20,panel.y+46);
    ...
    const drawSlot=({slot,x,y})=>{if(!slot)return;const occupant=items.find(item=>item.kind==='modifier'&&item.equippedSlot===slot.id),compatible=!active||Boolean(game.modifierEffect(chosen,slot)),selected=active&&target?.id===slot.id;ctx.fillStyle=selected?'rgba(214,255,63,.18)':compatible?'rgba(117,245,255,.08)':'rgba(255,255,255,.02)';ctx.fillRect(x,y,155,48);ctx.strokeStyle=selected?'#d6ff3f':compatible?'#75f5ff':'#354047';ctx.lineWidth=selected?3:1;ctx.strokeRect(x,y,155,48);ctx.fillStyle=compatible?'#ffffff':'#4c5b60';ctx.font='700 8px Space Mono';ctx.fillText(slot.label,x+10,y+17);ctx.fillStyle=occupant?'#d6ff3f':'#65757a';ctx.font='8px Space Mono';ctx.fillText(occupant?occupant.name.slice(0,18):compatible?'EMPTY':'INCOMPATIBLE',x+10,y+35);};slotBoxes.forEach(drawSlot);
    const storage={id:null,part:'storage',label:'STORAGE'},storageSelected=active&&target?.part==='storage';ctx.fillStyle=storageSelected?'rgba(214,255,63,.18)':'rgba(255,184,92,.06)';ctx.fillRect(755,450,155,38);ctx.strokeStyle=storageSelected?'#d6ff3f':'#80663d';ctx.lineWidth=storageSelected?3:1;ctx.strokeRect(755,450,155,38);ctx.fillStyle='#ffb85c';ctx.font='700 8px Space Mono';ctx.fillText(storage.label,765,465);ctx.fillStyle='#8a795e';ctx.fillText('REMOVE MODIFIER',765,480);
    const tooltip=game.equipmentTooltip();ctx.fillStyle='rgba(4,10,14,.94)';ctx.fillRect(535,515,600,102);ctx.strokeStyle=active?'#d6ff3f':'#75f5ff';ctx.lineWidth=2;ctx.strokeRect(535,515,600,102);this.drawItemIcon(game.selectedInventoryItem(),550,531,38);ctx.fillStyle='#ffffff';ctx.font='700 13px Space Mono';ctx.fillText(tooltip.name,600,540);ctx.fillStyle='#aab9bc';ctx.font='9px Space Mono';ctx.fillText(tooltip.detail,600,562);ctx.fillStyle=active?'#d6ff3f':'#75f5ff';ctx.font='700 9px Space Mono';ctx.fillText(tooltip.prompt,600,590);ctx.fillStyle='#71838a';ctx.font='8px Space Mono';ctx.fillText(`${items.length} ACQUIRED // ${game.player.body.internalSlots.length} INTERNAL BAYS`,140,615);if(rows.length>visibleCount){ctx.textAlign='right';ctx.fillText(`${game.inventorySelection+1} / ${rows.length}`,505,615);ctx.textAlign='left';}
  }
  ...
  drawMerchantMenu(game) {
    ...
    ctx.fillStyle='#71838a';ctx.font='700 10px Space Mono';ctx.fillText('MERCHANT CATALOG // I CLOSE // W S SELECT // O BUY',140,105);
  ...
  drawHud(game) {
    ...
    const merchant=game.nearbyMerchant();if(merchant){ctx.fillStyle='rgba(7,11,16,.92)';ctx.fillRect(410,626,460,48);ctx.strokeStyle=merchant.color;ctx.lineWidth=2;ctx.strokeRect(410,626,460,48);ctx.fillStyle='#ffffff';ctx.font='700 11px Space Mono';ctx.textAlign='center';ctx.fillText(`O  //  OPEN ${merchant.name} CATALOG`,640,654);ctx.textAlign='left';}
