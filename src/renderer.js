...
export class Renderer {
  ...
  drawEnemy(enemy,time) {
    ...
    if(enemy.isCrownBoss){const exposed=enemy.bossMove==='crownExpose',warning=enemy.bossMove?.endsWith('Windup'),relocating=enemy.bossMove==='crownRelocate';ctx.rotate(Math.sin(time*3)*.025);this.polygon([[-enemy.w/2+8,0],[-enemy.w*.34,-enemy.h/2],[0,-enemy.h*.36],[enemy.w*.34,-enemy.h/2],[enemy.w/2-8,0],[enemy.w*.32,enemy.h/2],[0,enemy.h*.34],[-enemy.w*.32,enemy.h/2]],relocating?'#18343c':'#1d2b31');ctx.strokeStyle=exposed?'#ffffff':warning?'#d6ff3f':'#75f5ff';ctx.lineWidth=5;ctx.strokeRect(-enemy.w/2+14,-enemy.h/2+10,enemy.w-28,enemy.h-20);ctx.fillStyle='#071116';ctx.fillRect(-29,-14,58,28);ctx.shadowBlur=exposed?30:warning?22:14;ctx.shadowColor=exposed?'#ffffff':warning?'#d6ff3f':'#75f5ff';ctx.fillStyle=exposed?'#ffffff':warning?'#d6ff3f':'#75f5ff';for(const x of[-17,0,17]){ctx.beginPath();ctx.arc(x,0,5,0,Math.PI*2);ctx.fill();}ctx.shadowBlur=0;for(const side of[-1,1]){this.line(side*enemy.w*.38,-4,side*enemy.w*.65,-18,'#8da1a6',5);this.polygon([[side*enemy.w*.65,-18],[side*enemy.w*.56,-29],[side*enemy.w*.52,-10]],'#d5e2e4');}ctx.restore();return;}
