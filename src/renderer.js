...
export class Renderer {
  ...
  drawEquipmentInventory(game,ctx) {
    ...
    const bodyX=830,bodyY=360;ctx.fillStyle='#1d2b31';this.polygon([[bodyX-72,bodyY-30],[bodyX-52,bodyY-50],[bodyX+48,bodyY-45],[bodyX+72,bodyY-20],[bodyX+62,bodyY+32],[bodyX-62,bodyY+35]],ctx.fillStyle);ctx.strokeStyle='#75f5ff';ctx.lineWidth=3;ctx.strokeRect(bodyX-42,bodyY-24,84,48);ctx.fillStyle='#071015';ctx.fillRect(bodyX-31,bodyY-12,62,23);for(const eyeX of[-18,0,18]){ctx.fillStyle='#ff493f';ctx.beginPath();ctx.arc(bodyX+eyeX,bodyY,5,0,Math.PI*2);ctx.fill();}this.line(bodyX-38,bodyY+30,bodyX-66,bodyY+67,'#87969b',5);this.line(bodyX,bodyY+30,bodyX-8,bodyY+70,'#87969b',5);this.line(bodyX+38,bodyY+30,bodyX+66,bodyY+67,'#87969b',5);this.line(bodyX+62,bodyY-6,bodyX+112,bodyY-6,'#dce6dd',6);
    const chosen=game.equipmentItemIndex===null?null:game.player.purchasedItems[game.equipmentItemIndex],target=game.selectedEquipmentTarget(),active=Boolean(chosen),bodySlots=game.bodySlots(),expansionSlots=game.player.body.expansionSlots,displayExpansionSlots=game.baySlotDefinitions().map(definition=>expansionSlots.find(slot=>slot.id===definition.id)??{...definition,locked:true}),normalSlotIds=new Set(game.normalSlots().map(slot=>slot.id)),slotBoxes=[{slot:bodySlots.find(slot=>slot.id==='shell'),x:555,y:270,w:150},{slot:bodySlots.find(slot=>slot.id==='core'),x:555,y:330,w:150},{slot:bodySlots.find(slot=>slot.id==='legs'),x:555,y:390,w:150},{slot:bodySlots.find(slot=>slot.id==='weapon'),x:955,y:330,w:150},...displayExpansionSlots.map((slot,index)=>({slot,x:720+index*130,y:250,w:120}))];
