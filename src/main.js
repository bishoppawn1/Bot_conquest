...
function begin() {
  ...
  if (debugSpawn === 'boss') {
    ...
    Object.assign(game.player,{scrap:860,primaryDamage:5,damageUpgrades:2,healthUpgrades:1,energyUpgrades:1,bayUpgrades:1,materials:{titanium:4,uranium:1},purchasedItems:[{id:'edge-coil-1',kind:'upgrade',name:'EDGE COIL MK 1',detail:'+1 PRIMARY SLASH DAMAGE'},{id:'edge-coil-2',kind:'upgrade',name:'EDGE COIL MK 2',detail:'+1 PRIMARY SLASH DAMAGE'},{id:'shell-capacity-1',kind:'upgrade',name:'SHELL CAPACITY MK 1',detail:'+1 MAX SHELL'},{id:'capacitor-bank-1',kind:'upgrade',name:'CAPACITOR BANK MK 1',detail:'+25 MAX ELECTRICITY'},{id:'bay-upgrade-1',kind:'upgrade',name:'AUXILIARY BAY A',detail:'REDUCED-EFFECT MODIFIER SLOT'},{id:'modifier-aegis-filament',modifierId:'aegis-filament',kind:'modifier',name:'AEGIS FILAMENT',equippedSlot:'internal'},{id:'modifier-reactive-governor',modifierId:'reactive-governor',kind:'modifier',name:'REACTIVE GOVERNOR',equippedSlot:'legs'},{id:'modifier-extender-arm',modifierId:'extender-arm',kind:'modifier',name:'EXTENDER ARM',equippedSlot:'weapon'},{id:'relic-feedback-dynamo',relicId:'feedback-dynamo',kind:'relic',name:'FEEDBACK DYNAMO',detail:'NORMAL-MOUNT RELIC // TAKING DAMAGE RESTORES 12 ELECTRICITY',equippedSlot:'shell'},{id:'relic-arc-retort',relicId:'arc-retort',kind:'relic',name:'ARC RETORT',detail:'NORMAL-MOUNT RELIC // TAKING DAMAGE HITS NEARBY ENEMIES FOR 2',equippedSlot:'core'}]});
    game.player.body.expansionSlots.push({id:'auxiliary-1',part:'auxiliary',label:'AUXILIARY BAY A',efficiency:.3});game.recomputeBodyStats();game.player.lives=4;game.player.electricity=125;game.player.shield=1;
  ...
  if(debugPhase==='2'){
    ...
    if(phaseBoss){phaseBoss.health=Math.floor(phaseBoss.maxHealth/2);phaseBoss.phase=1;phaseBoss.phaseShieldTime=0;}
  }
  playing = true;
  syncAbilityControls();
  renderer.resetLegs();
