...
export class Game {
  ...
  createBody(id,bayCount=0){const definition=this.shellDefinition(id);if(!definition)return null;return{id:definition.id,name:definition.name,slots:definition.slots.map(slot=>({...slot})),expansionSlots:BAY_SLOT_DEFINITIONS.slice(0,bayCount).map(slot=>({id:slot.id,part:slot.part,label:slot.label,efficiency:slot.efficiency})),loadout:{}};}
